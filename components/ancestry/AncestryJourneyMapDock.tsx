...
import { getAncestryJourneyStepPresentation } from '@/lib/ancestry-journey-dock-copy';
...
const AncestryJourneyMapDock = memo(function AncestryJourneyMapDock() {
  ...
  const isTruncated = planStepCount > maxSteps;
  const steps =
    plan && plan.steps.length > maxSteps ? plan.steps.slice(0, maxSteps) : plan?.steps;
  ...
  const presentation = useMemo(
    () => (currentStep ? getAncestryJourneyStepPresentation(currentStep, locale) : null),
    [currentStep, locale],
  );
  ...
  const truncationNote = isTruncated
    ? t('ancestryJourney.truncatedFootnote', locale)
        .replace('{shown}', String(maxSteps))
        .replace('{total}', String(planStepCount))
    : null;

  const bottomMobile = storyMode
    ? 'bottom-[max(10.5rem,calc(env(safe-area-inset-bottom)+9.5rem))]'
    : 'bottom-[max(5rem,calc(env(safe-area-inset-bottom)+4rem))]';
  const bottomDesktop = storyMode ? 'md:bottom-28' : 'md:bottom-6';

  return (
    <div
      className={`pointer-events-auto fixed ${bottomMobile} left-1/2 z-40 w-[min(34rem,calc(100vw-1rem))] -translate-x-1/2 rounded-xl border border-chrome-border-strong/55 bg-chrome-fill/95 px-3 py-2.5 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-md ${bottomDesktop}`}
      role="region"
      aria-label={t('ancestryJourney.dockTitle', locale)}
    >
      <div className="flex items-start gap-2">
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-gold/80">
            {t('ancestryJourney.dockTitle', locale)}
          </p>
          <p className="mt-0.5 text-[11px] text-text-dim">
            {progressTpl.replace('{current}', String(safeIndex + 1)).replace('{total}', String(total))}
          </p>
          {truncationNote ? (
            <p className="mt-0.5 text-[10px] leading-snug text-amber-200/90">{truncationNote}</p>
          ) : null}
          {presentation?.title ? (
            <p className="mt-1 line-clamp-2 text-[13px] font-medium leading-snug text-parchment/95">
              {presentation.title}
            </p>
          ) : null}
          {presentation?.body ? (
            <p className="mt-1 line-clamp-3 whitespace-pre-line text-[12px] leading-snug text-text-dim">
              {presentation.body}
            </p>
          ) : null}
        </div>
